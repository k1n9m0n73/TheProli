<?php
namespace App\Http\Controllers\Warehouse\Notification\Trait;
use Module\Escape;
...
private function getNot(){
  $mes  = DB::table("notify")->select(
    ...
    ->where(function($DB){
      $DB->where("sto",$this->user->user_id);
      $DB->orWhere("sto","Warehouse");
      $DB->orWhere("sto","all");
   ...
   //print_r($mes);
    ...
    if(count($mes ) > 0   ){
       echo json_encode(['data'=>$mes,"messaGE"]); 
  ...
  try {
    foreach($ids as $id){
      if(!is_numeric($id) ){
       ...
       if(!empty((array) $not)  &&   $not->sto =='Warehouse'){
  ...
  try {
    ...
    if(!empty((array) $not)  &&   $not->sto =='Warehouse'){
     ...
     if(!empty((array) $not)  &&   $not->sto =='all'){
      ...
      $del   =  true;// DB::table('notify')->where('id',$id)->delete();
       if(!$del){
         echo  json_encode(['err'=>'Error deleting notification, try again']);
       }else{
