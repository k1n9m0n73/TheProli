...
class ResetPasswordController extends Controller
{
   public function __construct()
   {
    $this->email ='';
    $this->token  = '';
    $this->component   = 'warehouse';
      $this->middleware("guest:war")->except($this->component."/login"); //the name of the guard in config/auth
    ...
    public function resetPassword(Request $request)
    {
        ...
        if($_POST['password']  !=$_POST['confirm_password']   ){
            return redirect($this->component."/password-reset/{$request->email}/{$request->token}")->withErrors('Password and repeat password are not equall');
            // echo json_encode(['err'=>'Password and repeat password are not equall']); 
             exit();
        }
            
          $user  = DB::table($this->component.'s')->where('email',$request->email)->where('remember_token', $request->token)->first();
        ...
        if (!empty($user)) {
           $token = substr(str_shuffle('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYS1234567890'),0,64);
                ...
                $time  =  (strtotime(Carbon::now()) - strtotime($user->created_at))/60 ;
                 ...
                 //echo $time ;
                 $time  < 20 ?DB::table($this->component.'s')->where('email',$request->input('email'))->update($data): redirect($this->component."/password-reset/{$request->email}/{$request->token}")->withErrors('token expired');
