...
use App\Http\Controllers\Warehouse\AdminControllerTrait\Order;
use App\Http\Controllers\Warehouse\AdminControllerTrait\Product;
...
class AdminUserData extends Controller
{
  ...
  //
    ...
    public function getTotalOrder(){
      ...
      try {
            $t  = DB::table("shop_orders")->where('item_store',$this->user->user_id)->count('id'); //code...
        ...
        if(!empty( (array)$doc ) ){
          ...
          if(empty($logDoc)){
                ...
                //  echo $diff.'<br/>';
                 if($diff  < 0 ){
                  ///////doccof  = 0;
                   ...
                   DB::table('warehouses')->where('user_id',$this->user->user_id)->update(['docconf',0]);
           ...
           echo json_encode(['data'=> [
             ...
             'willExp'=>$willExp,
             'hasExpr'=>$hasExp,
             'url' =>'/warehouse/settings/document#1'
             ]
            ]);
            exit;
            // print_r($docName);
