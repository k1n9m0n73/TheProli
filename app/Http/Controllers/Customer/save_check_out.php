...
class Checkout extends Controller{
    ...
    $user  = Customer::where('user_id',Session::get('user_customer'))->first();
    $this->_product = DB::table('store_contact')->where('id',1)->first(); 
...
exit();
  ...
  if(!empty($err)){
    ...
    Session::put('__new__cart__',$new_carts);
...
/************************************************************************************************************************************/ 
 ...
 private function routeOne($storage_structure_data,$customer_structure_data,$totalMass,$log_type,$partner_code){
   ...
   $PROLI_HUB = [];
    ...
    $hub_lga =  DB::table('hubs')->where('lga',trim($storage_structure_data['lga']) )->get();
    ...
    if (count($hub_lga )>0) {
      $PROLI_HUB = $hub_lga ;
       ...
       $PROLI_HUB =  DB::table('hubs')->where('state_id',trim($storage_structure_data['state_id']))->get();//////hub at state level
     }
...
->where('remainmass', '>' ,($totalMass/1000) )->where('has_approve',1)->where('log_type',3)
->where('vesava',1)->where('pvesloclga','regexp',trim($storage_structure_data['lga']))
->where('pveslocstate','regexp', trim($storage_structure_data['state']))
->where('dvesloclga','regexp',trim($THE_HUB['lga']))
...
if(count($log) ==0){
  ...
  ->where(function($q) use($storage_structure_data){
    $q->where('pvesloclga','regexp',trim($storage_structure_data['lga']));
    $q->orWhere('pvesloclga','regexp','All');
  })
  ->where(function($q) use($storage_structure_data){
    $q->where('pveslocstate','regexp', trim($storage_structure_data['state']));
    $q->orWhere('pveslocstate','regexp', 'All');
  
  } )
  ->where(function($q) use($THE_HUB){
    $q->where('dvesloclga','regexp',trim($THE_HUB['lga']));
...
if(count($log) >0) {
  ...
  $pat = [
    ...
    'delivery_cost'=> ($THE_HUB['distance_to_store']*$totalMass*$this->delivery_frac) < 1000?1200: ($THE_HUB['distance_to_store']*$totalMass*$this->delivery_frac),
    'route' =>'route_one',
 ...
 return ['details'=>$pat, 'from'=>$storage_structure_data,'to'=>$THE_HUB,'customer'=>$customer_structure_data,'deliverable'=>1];
}else{
...
private function routeTwo($hub_from,$customer_structure_data,$totalMass,$log_type,$partner_code){
  ...
  $hub_lga =  DB::table('hubs')->where('lga',trim($customer_structure_data['lga']))->get();
  if (count($hub_lga )>0) {
    $PROLI_HUB = $hub_lga ;
   }else{
...
$sortByDistance  =  function($a, $b) {
 ...
 if(empty($PROLI_HUB_RESTRICTURE)){
   ...
   ->where('vesava',1)->where('dvesloclga','regexp',trim($THE_HUB['lga']))
   ->where('dveslocstate','regexp', trim($THE_HUB['state']))
   ->where('pvesloclga','regexp',trim($THE_HUB_FROM['lga']))
  ...
  /// $sql = "select * from `vehicles` where `vescap` > '".($totalMass/1000)."' and `remainmass` > '".($totalMass/1000)."' and `has_approve` = 1 and `log_type` = 1 and `vesava` = 1 and `dvesloclga` LIKE '%".trim($THE_HUB['lga'])."%' and `dveslocstate` LIKE '%".trim($THE_HUB['state'])."%' and `pvesloclga` LIKE '%".trim($THE_HUB_FROM['lga'])."%' and `pveslocstate` LIKE '%".trim($THE_HUB_FROM['state'])."%' order by `delnum` asc";
   ...
   if(count($log) ==0){
     ...
     ->where(function($q) use($THE_HUB){
      ...
      $q->where('pvesloclga','regexp',trim($THE_HUB_FROM['lga']));
      $q->orWhere('pvesloclga','regexp','All');
    })->where(function($q) use($THE_HUB_FROM){
    ...
    //  print_r($pat);
      $pat = [
        ...
        'delivery_cost'=> ($THE_HUB['distance_to_customer']*$totalMass*$this->delivery_frac) < 1000?1200: ($THE_HUB['distance_to_customer']*$totalMass*$this->delivery_frac),
        'route' =>'route_one',
 ...
 //////////////////////////from hub two to customer
  ...
  //  print_r($pat);
    $pat = [
      ...
      'delivery_cost'=> ($customer_structure_data['distance_to_customer']*$totalMass*$this->delivery_frac) < 1000?1200: ($customer_structure_data['distance_to_customer']*$totalMass*$this->delivery_frac),
      'route' =>'route_one',
...
private function routeThree($hub_two,$customer,$totalMass,$partner_code){
  ...
  if(  ( !empty($hub_two) && !empty($customer) )  &&   ($hub_two['lga'] == $customer['lga'])  ){
    ...
    ->where('remainmass', '>' ,($totalMass/1000) )->where('has_approve',1)->where('log_type',4)->where('vesava',1)
    ->where(function($q) use($hub_two){
    $q->where('pvesloclga','regexp',trim($hub_two['lga']));
    $q->orWhere('pvesloclga','regexp','All');
  })->where(function($q) use($hub_two){
    $q->where('pveslocstate','regexp',trim($hub_two['state']));
    $q->orWhere('dveslocstate','regexp','All');
  })->where(function($q) use($customer){
    $q->where('dvesloclga','regexp',trim($customer['lga']));
    $q->orWhere('dvesloclga','regexp','All');
  })->where(function($q) use($customer){
  ...
  ->get();
    ...
    $pat2 = [
      ...
      'delivery_cost'=> ($customer['distance_to_customer']*$totalMass*$this->delivery_frac) < 1000?1200: ($customer['distance_to_customer']*$totalMass*$this->delivery_frac),
      'route' =>'route_one',
 ...
 private function routeFour($storage,$customer,$totalMass,$partner_code,$is_hub=false){
    ...
    if    (  (  !empty($storage['state']) ==  !empty($customer['state']) )  && ( $storage['state'] == $customer['state']  )    ){
      ...
      ->where(function($q) use($customer){
       ...
       $q->where('pvesloclga','regexp',trim($storage['lga']));
       $q->orWhere('pvesloclga','regexp','All');
     })->where(function($q) use($storage){
 ...
 $sortByDistance  =  function($a, $b) {
  ...
  if(empty($THE_HUB)){
    ...
    $path_code = 'L1-L2';
            ...
            if(count($log)==0) {   
              ...
              //  print_r($pat);
                $pat = [
                  ...
                  'address'=> $pat['veslocarea'] .' '. $pat['vesloclga']. ' '. $pat['veslocstate'],
                  'log_id'=>$pat['log_id'],
                  'THE_HUB_id'=>$THE_HUB['id'],
                  'hub_id'=>$storage['id'],
                  'partner_code'=>$partner_code.'__'.$path_code, 
                  'mass'=>$totalMass,
                  'distance'=>$THE_HUB['distance_to_THE_HUB'],
                  'delivery_cost'=> ($THE_HUB['distance_to_THE_HUB']*$totalMass*$this->delivery_frac) < 1000?1200: ($THE_HUB['distance_to_THE_HUB']*$totalMass*$this->delivery_frac),
                  'route' =>'route_one',
            ...
            /////////gho ahead find the logistic 4 that can deliver item to customer/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            ->where('remainmass', '>' ,($totalMass/1000) )->where('has_approve',1)->where('log_type',4)->where('vesava',1)
            ->where(function($q) use($THE_HUB){
            $q->where('pvesloclga','regexp',trim($THE_HUB['lga']));
            $q->orWhere('pvesloclga','regexp','All');
          })->where(function($q) use($THE_HUB){
...
public function useAddress(){
 ...
 if (isset($_POST['post0'])) {
  ...
  $data2=[
    'city'=>$ad->collector_lga,
    'city_id'=>$ad->collector_lga,
