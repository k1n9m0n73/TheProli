...
class SearchItems extends Controller{
    ...
    {    
           ...
           return view('customer.components.Item_search_by_letter',['user'=> $this->user ]) ;
           ...
           foreach (json_decode($_POST['post0']) as $key => $value) {
            ...
            //  $kvp .= "item_".Escape::done($key) ." BETWEEN ". preg_replace("#-#", " AND ", Escape::done($value) )." AND "  ; 
             ...
             //  $kvp  .= "item_vendor_".Escape::done($key)."='".preg_replace("#-#", " ", Escape::done($value) )."' AND " ;# code...
                ...
                $item_category = $item_category->where(function($q) use($value){
                    $q->where("item_name","regexp",Escape::done($value));
                    $q->orWhere("item_type","regexp", Escape::done($value));
