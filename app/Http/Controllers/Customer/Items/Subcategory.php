...
class Subcategory extends Controller{
    ...
    {    
           ...
           return view('customer.components.subcategory_items',['user'=> $this->user ]) ;
   ...
   {     
    ...
    "item_store.item_images AS itimg","item_store.item_rating AS itrt","item_vendor_state AS itvs",
    "item_store.item_name AS itn","item_store.item_discount As itdct","item_harvest_day AS ithd"
    )
        ->where("conf", 1)
        ->where('item_qty', '>', 0)
           ...
           foreach (json_decode($_POST['post0']) as $key => $value) {
            ...
            //  $kvp .= "item_".Escape::done($key) ." BETWEEN ". preg_replace("#-#", " AND ", Escape::done($value) )." AND "  ; 
             ...
             //  $kvp  .= "item_vendor_".Escape::done($key)."='".preg_replace("#-#", " ", Escape::done($value) )."' AND " ;# code...
                ...
                $item_category = $item_category->where("item_subcateId", Escape::done($value));
             //  $kvp  .= "item_cateId"."='".Escape::done($value)."' AND " ;# code...
              }else{
