        ...
        private function orderAnalytic($request){
            ...
            $item = DB::table('shop_orders')->select(
             ...
             //   print_r($item);   
              ...
              if(!empty( $request->input('age_from') ) && !empty( $request->input('age_to') )){
                $item  = $item->whereBetween('age',[$request->input('age_from'),$request->input('age_to')]);
              }else

              if(!empty( $request->input('age_from') ) ){
                $item  = $item->where('age',$request->input('age_from'));
              }

              if(!empty( $request->input('ltype') ) && !empty($request->input('log_user')) ){
                   $m  = [
                       'Log1' =>'log1',
                       'Log2' =>'log2',
                       'Log3a' =>'log3a',
                ...
                $item  = $item->where($m[$request->input('ltype')].'_id','regex',Escape::done($request->input('log_user')) );
              }else  if(!empty( $request->input('ltype') ) ){
                $m  = [
                    'Log1' =>'log1',
                    'Log2' =>'log2',
                    'Log3a' =>'log3a',
             ...
             $item  = $item->where($m[$request->input('ltype')].'_id',"!=", "NULL" );
           }else

              if(!empty( $request->input('gender') ) ){
                $item  = $item->where('gender',$request->input('gender'));
              }
             ...
             if(!empty( $request->input('lga') )){
                // $state  = explode("__#__" ,Escape::done($request->input('state')));
                 $lga  = explode("__#__" ,Escape::done($request->input('lga')));
               
                $item  =$item->where(function($q) use($lga){
                    $q->where('lga',$lga[1] );
                    $q->orWhere('lga','regexp', $lga[1]);
                    //$q->where('category_id',Escape::done($request->input('cate')));
               ...
               if(!empty( $request->input('state') )){
                $state  = explode("__#__" ,Escape::done($request->input('state')));
               // $lga  = explode("__#__" ,Escape::done($request->input('lga')));
               $item  =$item->where(function($q) use($state){
                     $q->where('state',$state[2] );
                     $q->orWhere('state','regexp',$state[2]);
                    // $q->where('category_id',Escape::done($request->input('cate')));
