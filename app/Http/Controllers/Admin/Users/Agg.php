    ...
    {  
         ...
         $this->middleware(function ($request, $next ) {
          ...
          $this->user_perm = [
                              ...
                              property_exists($permission,'ppar__allow_to_view_aggregator') && $permission->ppar__allow_to_view_aggregator==1?1:0,
         ...
         $this->user_gperm  =   [
          ...
          property_exists($gpermission,'ppar__allow_to_view_aggregator') && $gpermission->ppar__allow_to_view_aggregator==1?1:0,
          property_exists($gpermission,'ppar__allow_to_edit_aggregator') && $gpermission->ppar__allow_to_edit_aggregator==1?1:0,
          property_exists($gpermission,'ppar__allow_to_delete_aggregator') && $gpermission->ppar__allow_to_delete_aggregator==1?1:0,
          property_exists($gpermission,'papp__allow_to_approve_aggregator') && $gpermission->papp__allow_to_approve_aggregator==1?1:0,
     ...
     public function getNewView()
     {
          if($this->user_perm[0]==1 && $this->user_gperm[0]==1){
              return view('admin.component.users.agg.new',['user'=>$this->user]);
          } else{
     ...
     {   if($this->user_perm[0]==1 && $this->user_gperm[0]==1){
         return view('admin.component.users.agg.approved',['user'=>$this->user]);
        }else{
          return view('welcome',['denied'=>"Permission denied"]);
        }
     } 
     ...
     {   
        ...
        if($this->user_perm[1]==1 && $this->user_gperm[1]==1){
          $user_part = DB::table('aggregators')->where("user_id",$id)->first();
                  $acc = DB::table('bankacc')->where("user_id",$id)->first();
                
                  
                return view('admin.component.users.agg.details',[
                    'user'=>$this->user,
                    'id'=>$id,
                    'user_data'=>$user_part,
     ...
     public function getadminData(Request $request,$id=null){
          ...
          $sql  = DB::table('aggregators')->select('user_id AS a','fn AS b','mn AS c','ln AS d','email  AS e','created_at AS f','pn AS g','joined AS h','img AS i')
  ...
  //print_r($sql->toSql());
    ...
    //  print_r($id);
     ...
     private function approve($req,$data,$app=true){
         ...
         try {  $d =  DB::table('aggregators')->where('user_id',$req->input('post0'))->update($data);
        if($d){
        ...
        $table  = 'aggregators';
            try {
                    foreach ($p as $key_ =>$value_) { 
                        ...
                        $user  =DB::table($table)->select('user_id','img','documents')->where('user_id',$value_)->first();
