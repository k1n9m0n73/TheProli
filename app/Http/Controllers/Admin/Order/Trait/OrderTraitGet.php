...
namespace App\Http\Controllers\Admin\Order\Trait;
...
public function getTodayOrder(Request $request){
  $month_map  = [
    ...
    $item  =$item->take($request->input('post2'))->skip($request->input('post1'))
   //->toSql();
   ->get();
...
private function getHub($req){
  ...
  private function cancel_($req){
  
      //////////////////////////get the item
      try {
        $item =  DB::table('shop_orders')->where('data_id',Escape::done($_POST['post2']))->first();
   ////////////////////////////////////////////////////////////////////////////////////////////  
        if(!empty((array)$item  )  ){
            if($item->order_status=='cancel'){
              echo  json_encode(['err'=>'The order is already cancelled']);
              exit();
            }
          DB::table('shop_orders')->where('data_id',Escape::done($_POST['post2']))->update(['order_status'=>'']);



           $d  =   DB::table('shop_orders')->where('data_id',Escape::done($_POST['post2']))->update(['order_status'=>'cancel']);
            $value  = (array)$item;
           if($d){
            DB::table('partner_settlement')->where('order_id', $item->order_id)->update(['transaction_status'=>0]);
               //////////////////////////is updted
               if(!empty($value['log1_id'])){
                Notification::sendNotification($value['customer_id'],$value['log1_id'], "You have an order");  
                ...
                if(!empty( (array)$theVeh )){
                    ...
                    'loadedmass'  => $theVeh->loadedmass+$value['titem_mass'],
              ...
              if(!empty($value['log2_id'])){
                Notification::sendNotification($value['customer_id'],$value['log2_id'], "You order has been cancel");  
               ...
               if(!empty($value['log3a_id'])){
                Notification::sendNotification($value['customer_id'],$value['log3a_id'], "You order has been cancel");  
               ...
               if(!empty($value['log3b_id'])){
                Notification::sendNotification($value['customer_id'],$value['log3b_id'], "You order has been cancel");  
               ...
               if(!empty($value['log4a_id'])){
                Notification::sendNotification($value['customer_id'],$value['log4a_id'], "You order has been cancel");  
               ...
               if(!empty($value['log4b_id'])){
                Notification::sendNotification($value['customer_id'],$value['log4b_id'], "You order has been cancel");  
               ...
               if(!empty($value['item_uploader'])){
                Notification::sendNotification($value['customer_id'],$value['item_uploader'], "You order has been cancel");  
               }
           
               if(!empty($value['item_store'])){
                Notification::sendNotification($value['customer_id'],$value['item_store'], "You order has been cancel");  
               }
              
               Notification::sendNotification($value['customer_id'],$value['item_owner'], "You order has been cancel");  
               Notification::sendNotification($value['customer_id'],'admin', "You order has been cancel"); 
            ...
            echo  json_encode(['suc'=>'order cancel successfully','url'=>' ']);
            exit();
           }else{
            echo  json_encode(['err'=>'Error procesing request']);
            exit();
           }
///////////////////////////////////////////////////////
  ...
  private function reorder_($req){
    
      try {
        $order  = DB::table('shop_orders')->where('data_id',$req->input('post2'))->first();
         

          if(!empty( (array)$order )){
            /////////////////////////////////////
            if($order->order_status !='cancel'){
              echo  json_encode(['err'=>'The order is not cancelled yet']);
              exit();
            }
            ...
            $d  =   DB::table('shop_orders')->where('data_id',Escape::done($_POST['post2']))->update(['order_status'=>'Still in waerehouse']);
            ...
            if($d){
              ...
              if(!empty($value['log2_id'])){
               Notification::sendNotification($value['customer_id'],$value['log2_id'], "You have an order");  
               $veh_id  = json_decode(json_decode($value['log2_details']))->vehid;
               $theVeh  = DB::table('vehicles')->select('delnum','loadedmass','vescap')->where('data_id',$veh_id)->first(); 
               if(!empty( (array)$theVeh )){
               $veh_data  = [
              ...
              if(!empty($value['log3a_id'])){
               Notification::sendNotification($value['customer_id'],$value['log3a_id'], "You have an order");  
               $veh_id  = json_decode(json_decode($value['log3a_details']))->vehid;
               $theVeh  = DB::table('vehicles')->select('delnum','loadedmass','vescap')->where('data_id',$veh_id)->first(); 
               if(!empty( (array)$theVeh )){
               $veh_data  = [
              ...
              if(!empty($value['log3b_id'])){
               Notification::sendNotification($value['customer_id'],$value['log3b_id'], "You have an order");  
               $veh_id  = json_decode(json_decode($value['log3b_details']))->vehid;
               $theVeh  = DB::table('vehicles')->select('delnum','loadedmass','vescap')->where('data_id',$veh_id)->first(); 
               if(!empty( (array)$theVeh )){
               $veh_data  = [
              ...
              if(!empty($value['log4a_id'])){
               Notification::sendNotification($value['customer_id'],$value['log4a_id'], "You have an order");  
               $veh_id  = json_decode(json_decode($value['log4a_details']))->vehid;
               $theVeh  = DB::table('vehicles')->select('delnum','loadedmass','vescap')->where('data_id',$veh_id)->first(); 
               if(!empty( (array)$theVeh )){
               $veh_data  = [
              ...
              if(!empty($value['log4b_id'])){
               Notification::sendNotification($value['customer_id'],$value['log4b_id'], "You have an order");  
               $veh_id  = json_decode(json_decode($value['log4b_details']))->vehid;
               $theVeh  = DB::table('vehicles')->select('delnum','loadedmass','vescap')->where('data_id',$veh_id)->first(); 
               if(!empty( (array)$theVeh )){
               $veh_data  = [
              ...
              if(!empty($value['item_uploader'])){
               Notification::sendNotification($value['customer_id'],$value['item_uploader'], "You have an order");  
              }
          
              if(!empty($value['item_store'])){
               Notification::sendNotification($value['customer_id'],$value['item_store'], "You have an order");  
              }
             
              Notification::sendNotification($value['customer_id'],$value['item_owner'], "You have an order");  
              Notification::sendNotification($value['customer_id'],'admin', "You have an order");
              echo  json_encode(['suc'=>'order creplaced successfully','url'=>' ']);



            } else{
              echo  json_encode(['err'=>'Error processing request, try again later']);
              exit();
            }  
    ...
    if(isset($_POST['post0'])){
      ...
      //  print_r($ids);
       foreach ($ids as $key => $id) {
        ...
        try {
          $item =  DB::table('shop_orders')->where('data_id',$id)->first();
     ////////////////////////////////////////////////////////////////////////////////////////////  
          if(!empty((array)$item  )  ){
             ...
             if($d){
                 ...
                 //////////////////////////is updted
                 if(!empty($value['log1_id'])){
                  Notification::sendNotification($value['customer_id'],$value['log1_id'],
                   "Item with order ID ".$item->order_id." has been deleted ");  
                  //print_r(json_decode(json_decode($value['log1_details'])) );
                  $veh_id  = json_decode(json_decode($value['log1_details']))->vehid;
                  $theVeh  = DB::table('vehicles')->select('delnum','loadedmass','vescap')->where('data_id',$veh_id)->first(); 
                ...
                if(!empty($value['log2_id'])){
                  Notification::sendNotification($value['customer_id'],$value['log2_id'], 
                  "Item with order ID ".$item->order_id." has been deleted ");  
                  $veh_id  = json_decode(json_decode($value['log2_details']))->vehid;
                  $theVeh  = DB::table('vehicles')->select('delnum','loadedmass','vescap')->where('data_id',$veh_id)->first(); 
                  if(!empty( (array)$theVeh )){
                 ...
                 if(!empty($value['log3a_id'])){
                  Notification::sendNotification($value['customer_id'],$value['log3a_id'],
                  "Item with order ID ".$item->order_id." has been deleted ");  
                  $veh_id  = json_decode(json_decode($value['log3a_details']))->vehid;
                  $theVeh  = DB::table('vehicles')->select('delnum','loadedmass','vescap')->where('data_id',$veh_id)->first(); 
                  if(!empty( (array)$theVeh )){
                 ...
                 if(!empty($value['log3b_id'])){
                  Notification::sendNotification($value['customer_id'],$value['log3b_id'], 
                  "Item with order ID ".$item->order_id." has been deleted ");  
                  $veh_id  = json_decode(json_decode($value['log3b_details']))->vehid;
                  $theVeh  = DB::table('vehicles')->select('delnum','loadedmass','vescap')->where('data_id',$veh_id)->first(); 
                  if(!empty( (array)$theVeh )){
                 ...
                 if(!empty($value['log4a_id'])){
                  Notification::sendNotification($value['customer_id'],$value['log4a_id'], 
                  "Item with order ID ".$item->order_id." has been deleted ");  
                  $veh_id  = json_decode(json_decode($value['log4a_details']))->vehid;
                  $theVeh  = DB::table('vehicles')->select('delnum','loadedmass','vescap')->where('data_id',$veh_id)->first(); 
                  if(!empty( (array)$theVeh )){
                 ...
                 if(!empty($value['log4b_id'])){
                  Notification::sendNotification($value['customer_id'],$value['log4b_id'],
                  "Item with order ID ".$item->order_id." has been deleted ");  
                  $veh_id  = json_decode(json_decode($value['log4b_details']))->vehid;
                  $theVeh  = DB::table('vehicles')->select('delnum','loadedmass','vescap')->where('data_id',$veh_id)->first(); 
                  if(!empty( (array)$theVeh )){
                 ...
                 if(!empty($value['item_uploader'])){
                  Notification::sendNotification($value['customer_id'],$value['item_uploader'], 
                  "Item with order ID ".$item->order_id." has been deleted ");  
                 }
                 ...
                 DB::table('shop_orders')->where('data_id',$id)->delete();
                 DB::table('partner_settlement')->where('order_id', $item->order_id)->delete();
                 //////////////////////////////////is updated
  
  
  
  
              echo  json_encode(['suc'=>'order deleted successfully','url'=>' ']);
              exit();
             }else{
              echo  json_encode(['err'=>'Error procesing request']);
              exit();
