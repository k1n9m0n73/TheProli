<?php
namespace App\Http\Controllers\Aggregator\Order\Trait;
...
public function getTodayOrder(Request $request){
  $month_map  = [
    ...
    $item  =$item->where('item_uploader',$this->user->user_id)->take($request->input('post2'))->skip($request->input('post1'))
   //->toSql();
   ->get();
