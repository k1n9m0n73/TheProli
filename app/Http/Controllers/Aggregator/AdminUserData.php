...
use App\Http\Controllers\Aggregator\AdminControllerTrait\Order;
use App\Http\Controllers\Aggregator\AdminControllerTrait\Product;
use App\Http\Controllers\DateTimes;

class AdminUserData extends Controller
{
    use Product,Order,DateTimes;
  //
    ...
    public function getTotalOrder(){
      ...
      try {
            $t  = DB::table("shop_orders")->where('item_uploader',$this->user->user_id)->count('id'); //code...
        ...
        if(!empty( (array)$doc ) ){
          ...
          if(empty($logDoc)){
                ...
                //  echo $diff.'<br/>';
                 if($diff  < 0 ){
                  ///////doccof  = 0;
                   ...
                   DB::table('aggregators')->where('user_id',$this->user->user_id)->update(['docconf',0]);
           ...
           echo json_encode(['data'=> [
             ...
             'willExp'=>$willExp,
             'hasExpr'=>$hasExp,
             'url' =>'/aggregator/settings/document#1'
             ]
            ]);
            exit;
            // print_r($docName);
