...
class RegistrationController extends Controller
{
 use DateTimes;
  ...
  {
     $this->dbtable  = "aggregators";
    $this->middleware("guest:agg")->except("aggregator/login"); //the name of the guard in config/auth
    ...
    {  
        ...
        try {
          ...
          $doc_data  = count( $doc) >0 ? $doc[0]->aggregator: json_encode([]);
          return view('aggregator.register',['data'=>['doc'=>$doc_data]]); //code...
    ...
    public function sendMail($subject,$message,$to,$token){
      ...
      $details = [
          ...
          'link'=> route('aggregator').'/getting_started?email='.$to.'&token='.$token.'',
...
$selected_city  = ['zone_id'=>$selected_state_[1] ,'state_id'=>$selected_lga_[0],'lga_name'=>$selected_lga_[1],
'name'=>$selected_lga_[1],'latitude'=>$selected_area_[0],
'longitude'=>$selected_area_[1],'area_name'=>$selected_area_[2]
];
...
$messagebody=$request->input('bn'). '; aggregator, this is to inform you that your registration details has been received in PROLI system. To complete this registration, it is necessary to confirm your email within 24 hours of registration<br><br>WHAT IS NEXT?<br>
...
$data =       [          'bn'=>  Escape::done($request->input('bn')),
                      ...
                      //  'adm_id' =>$batch.'s',//////////////f=>farmer
                        'user_id' =>$batch.'_aggregator',//////////////f=>farmer
                       // 'bkid' =>$bkid,
                        ...
                        'year' => date('Y',strtotime( $this->actday() )),
 ...
 ////////////////////////////////////////////
    ...
    try {
      if($this->sendMail($subject , $messagebody,$to,$token)){
       Aggregators::create($data);
       // DB::table($admin)->insert($data);
      Notification::sendNotification($batch.'_aggregator','admin', "New aggregator jsut register");
     // echo "dovbne";
     echo json_encode(['suc'=>'Registration successful, confirm your email','url'=>'/aggregator/login']);
  ...
  public function gettingStarted($a=null){
    ...
    return view('aggregator.started',['data'=>$a]);
      ...
      $token = Escape::done($_POST['post1']);
       ...
       $user_acc = count($user) >0 ? DB::table('bankacc')->where("user_id",$user[0]->user_id)->first():[];
          ...
          //  $_SESSION['partner'] = $user[0]->email;
             ...
             if (!empty($user_acc)  ) {
              echo  json_encode(['suc'=>'Email confirmed,bank detail exist , login ', 'url'=>'/aggregator/login']);
           exit();
             }else{
                 echo  json_encode(['suc'=>'Email confirmed, Supply your bank details', 'url'=>'/aggregator/getting_started/bank-details/'.$user[0]->user_id.'?bank']);
           exit();
             }
        }       
           ...
           if ($user[0]->tk != $token  ) {
            echo json_encode(['err'=>'Token is no more valid ','url'=>'/aggregator/getting_started/regenerate-token/'.$user[0]->user_id.'?regenerate']);
            exit();
         }
  ...
  public function regenerateTokens(Request $request)
  {
      ...
      if(empty($request->input('email'))){
        ...
        $user  = DB::table('aggregators')->where('email',$request->input('email'))->first();
     
      if (!empty($user)) {
        $mess  = "".$user->fn." ".$user->ln." has requested for email re-confirmation of aggregator account on THE PROLI. Click the link below to reset it ";  
         $token = substr(str_shuffle('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYS1234567890'),0,64);
          if($this->sendMail("Password reset request",$mess,$request->input('email'),$token)){
              ...
              DB::table('aggregators')->where('email',$request->input('email'))->update($data);
          }
  ...
  DB::table('bankacc')->insert($data);  
    ...
    echo json_encode(['suc'=>'Congratulation, detail save. Proceed','url'=>'/aggregator/login']);
