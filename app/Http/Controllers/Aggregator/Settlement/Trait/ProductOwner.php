<?php
namespace App\Http\Controllers\Aggregator\Settlement\Trait;
use Module\Escape;
...
private function productUploaderSettlement($request){
    ...
    if (isset($_POST['post0'])) {
     ...
     $set =Escape::done( $_POST['cate']-1); 
         ...
         $items  = $items->where("partner_settlement.aggregator_id", $this->user->user_id)->orderByDesc("partner_settlement.id")
         ->take(Escape::done($_POST['post2']))->skip(Escape::done($_POST['post1']))
        // ->toSql();
        // ->getBindings();
         ->get();
