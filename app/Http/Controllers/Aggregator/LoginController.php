...
class LoginController extends Controller
{
   public function __construct()
   {
     ...
     $this->middleware('guest:agg', ['except' =>"aggregator/logout"  ]);
    ...
    public function customLogin(Request $request)
    {
       ...
       if(empty($email_check) ){
        ...
        if(!empty($email_check) && $email_check->conf==0 ){
            return redirect('/aggregator/getting_started/bank-details/'.$email_check->user_id.'?regenerate')->withErrors('Email yet to be confirmed');
        }
        ...
        if (empty($acc) ) {
           // echo  json_encode(['err'=>'Your bank detail is required','url'=>'/aggregator/getting_started/bank-details/'.$email_check->user_id.'?bank']);
            return redirect('/aggregator/getting_started/bank-details/'.$email_check->user_id.'?bank')->withErrors('Your bank detail is required');
            //exit();
           } 
        ...
        $rem   = true;
        if (Auth::guard('agg')->attempt($credentials,$rem)) {
