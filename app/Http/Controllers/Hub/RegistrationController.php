...
class RegistrationController extends Controller
{
  ...
  {
     $this->dbtable  = "Hubs";
     $this->middleware("guest:war")->except("Hub/login"); //the name of the guard in config/auth
    ...
    {  
        ...
        $doc_data  = count( $doc) >0 ? $doc[0]->Hub:json_encode([]);
        return view('Hub.register',['data'=>['doc'=> $doc_data]]);
    }
    ...
    public function sendMail($subject,$message,$to,$token){
      ...
      $details = [
          ...
          'link'=> route('Hub').'/getting_started?email='.$to.'&token='.$token.'',
...
$messagebody=$request->input('bn'). ' Hub, this is to inform you that your registration details has been received in PROLI system. To complete this registration, it is necessary to confirm your email within 24 hours of registration<br><br>WHAT IS NEXT?<br>
...
$data =       [          'bn'=>  Escape::done($request->input('bn')),
 ...
 ////////////////////////////////////////////
    ...
    try {
      ...
      Notification::sendNotification($batch.'_Hub','Hub', "New aggregator jsut register");
     // echo "dovbne";
     echo json_encode(['suc'=>'Registration successful, confirm your email','url'=>'/Hub/login']);
  ...
  public function gettingStarted($a=null){
    ...
    return view('Hub.started',['data'=>$a]);
        ...
        $user_acc = count($user) >0 ?DB::table('bankacc')->where("user_id",$user[0]->user_id)->first():[];
          ...
          if ( count($user) >0 && $user[0]->conf == 1) {
           ...
           //$_SESSION['partner'] = $user[0]->email;
             ...
             if (!empty($user_acc)  ) {
              echo  json_encode(['suc'=>'Email confirmed,bank detail exist , login ', 'url'=>'/Hub/login']);
           exit();
             }else{
                 echo  json_encode(['suc'=>'Email confirmed, Supply your bank details', 'url'=>'/Hub/getting_started/bank-details/'.$user[0]->user_id.'?bank']);
           exit();
             }
        }       
           ...
           if ($user[0]->tk != $token  ) {
            echo json_encode(['err'=>'Token is no more valid, unknown token ','url'=>'/Hub/getting_started/regenerate-token/'.$user[0]->user_id.'?regenerate']);
            exit();
         }
  ...
  // exit();
     if ($td>24) {
      echo json_encode(['err'=>'Token is no more valid ','url'=>'/Hub/getting_started/regenerate-token/'.$user[0]->user_id.'?regenerate']);
      exit();
  ...
  public function regenerateTokens(Request $request)
  {
      ...
      if(empty($request->input('email'))){
        ...
        $user  = DB::table('Hubs')->where('email',$request->input('email'))->first();
        ...
        $mess  = "".$user->bn." has requested for email re-confirmation of aggregator account on THE PROLI. Click the link below to reset it ";  
         $token = substr(str_shuffle('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYS1234567890'),0,64);
          if($this->sendMail("Password reset request",$mess,$request->input('email'),$token)){
              ...
              DB::table('Hubs')->where('email',$request->input('email'))->update($data);
          }

          echo  json_encode(['suc'=>'Email sent', 'url'=>'/Hub/login']);
  ...
  DB::table('bankacc')->insert($data);  
    ...
    echo json_encode(['suc'=>'Congratulation, detail save. Proceed','url'=>'/Hub/login']);
