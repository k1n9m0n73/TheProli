<?php
namespace App\Http\Controllers\Hub\Message\Trait;
use Module\Escape;
        ...
        foreach( $to_emails as $email){
             if($email != 'admin'){  
              $chk    = DB::table($db_user_map[$_POST['parner']])->select('user_id')
                ...
                if(empty( (array)$chk ) ){
                    echo json_encode(['err'=>"The sytem abort sending the messge, email: ".$email." is not part of " .$db_user_map [$_POST['parner']] ]);
                exit();
                }else{
