<?php
namespace App\Http\Controllers\Hub\Settlement\Trait;
use Module\Escape;
...
{ 
        ...
        private function hub1Settlement($request){
           ...
           $set =Escape::done( $_POST['cate']-1); 
               ...
               $items  = $items->where("partner_settlement.hub1", $this->user->user_id)->orderByDesc("partner_settlement.id")
        ...
        private function hub2Settlement($request){
         ...
         $set =Escape::done( $_POST['cate']-1); 
             ...
             $items  = $items->where("partner_settlement.hub2", $this->user->user_id)->orderByDesc("partner_settlement.id")
             ->take(Escape::done($_POST['post2']))->skip(Escape::done($_POST['post1']))
            // ->toSql();
            // ->getBindings();
             ->get();
      ...
      private function hub3Settlement($request){
       ...
       $set =Escape::done( $_POST['cate']-1); 
           ...
           $items  = $items->where("partner_settlement.hub3", $this->user->user_id)->orderByDesc("partner_settlement.id")
           ->take(Escape::done($_POST['post2']))->skip(Escape::done($_POST['post1']))
          // ->toSql();
          // ->getBindings();
           ->get();
