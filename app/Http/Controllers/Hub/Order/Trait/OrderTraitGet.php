<?php
namespace App\Http\Controllers\Hub\Order\Trait;
...
public function getTodayOrder(Request $request){
  $month_map  = [
    ...
    $item  =$item->where(function($DB){
      $DB->where('hub1','regexp',   $this->user->user_id);
      $DB->orWhere('hub2','regexp',$this->user->user_id);
      $DB->orWhere('hub3','regexp',$this->user->user_id);
...
private function getHub($req){
  ...
  public function toFromLogResponse($item){
  ...
  //  print_r($_POST);
    ...
    if (empty($_POST['puvc'])   && $_POST['_what'] =='1') {
      ...
      if (empty($_POST['log_id'])   && $_POST['_what'] =='2') {
     
        echo json_encode(['err'=>" Logistics is required"]);
        exit();
        }
      ...
      if (empty($_POST['type'])) {
        ...
        ->where($log_map[Escape::done($_POST['type'])],'regexp',$this->user->user_id)
        ->where($log_map[Escape::done($_POST['status'])].'_id','!=','NULL')
       // ->toSql();
        ->first(); // DB::getInstance()->get("shop_orders",['puvc = "'.$_POST['puvc'].'" AND data_id','=',$item]) ;
     // print_r($order);
     
        if (count((array)$order) ==0) {
          echo json_encode(['err'=>"Check your PUVC or your hub type or the logistics type is not the one"]);
        exit();
        }
     ...
     try {
      ...
      $puvc=   $order->puvc.'_'.$batch.'__hub'.$log_map[Escape::done($_POST['type'])];////explode by __log to get prevous log type
      ...
      // print_r($pucv_arr);
          ...
          ////////////////////////////////////////////////////check log details;
          $d_  = $log_map[Escape::done($_POST['status'])].'_details';
        ...
        // print_r( $d_);
         ...
         //  print_r($veh_details);
           ...
           $d  = [
            ...
            'puvc'=>$puvc,
            'del_sta'=>'Item is now at '. $veh_details['statet'] .' '.$veh_details['lgat'].' '.$veh_details['areat'], //.' and is going to '. $veh_details['statet'] .' '.$veh_details['lgat'],
           // $d_ => json_encode(json_encode($veh_details)),
          // 'order_status'=>'incompleted',
            $resp_=> !empty($order->$resp_)? $order->$resp_.'___'.$veh_details['log_id']:$veh_details['log_id'],////the user_id that give to them
          ];
          ...
          // $this->user->update("shop_orders",$order->_results[0]->id,['log1_res'=>$_POST['puvc'],'del_sta'=>"picked from warehouse"]);
             ...
             //////////////////you want to deliver to log
             ////give log your hub code
             ////collect log puvc
             ////give us who you are e.g hub1 hub2 or hub3 ..
          
              $log_id   = Escape::done($_POST['log_id']);
             $order  =  DB::table('shop_orders')
             ->where('data_id',Escape::done($item))
              //->where('puvc',Escape::done($_POST['puvc']))
             ->where($log_map[Escape::done($_POST['type'])],'regexp',$this->user->user_id)
             ->where($log_map[Escape::done($_POST['status'])].'_id','regexp',$log_id)
           //  ->toSql();
              //->getBindings();
             ->first(); // DB::getInstance()->get("shop_orders",['puvc = "'.$_POST['puvc'].'" AND data_id','=',$item]) ;
       //    print_r($order);//
             ...
             if (count((array)$order) ==0) {
               echo json_encode(['err'=>"The logistics do not match the type to receive the item"]);
             exit();
             }
      ...
      // print_r($pucv_arr);
        ...
        //  print_r( $d_);
         ...
         //  print_r($veh_details);
           ...
           $d  = [
            $log_map[Escape::done($_POST['type'])].'_res'=>!empty($order->$resp_)?$order->$resp_.'___'.$_POST['log_id'].'__YES__':$_POST['log_id'].'__YES__',
...
// print_r($logs);
// echo $req->input('type');
     if($last_hub != $log_map[Escape::done($req->input('type'))]  )  {
      echo  json_encode(['err'=>'You are not the hub that will do the delivery to customer']);
      exit();
     }
     ...
     $puvc=   $order->puvc.'_'.$batch.'__cus'.$order->customer_id;
        ...
        $resp_  = $log_map[Escape::done($_POST['type'])].'_res';
         ...
         $d  = [
          $resp_=>!empty($order->$resp_)?$order->$resp_.'___'.$order->customer_id:$order->customer_id,
          'puvc'=>$puvc,
     ...
     else if($req->input('_what') == "1"){///receive
       //////////////take pdc' if true
      ///////transaction_satus  = 1;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 private function getOrderReceiveCode($request){
