        ...
        if(isset($_POST['post0'])){
          $ob   = json_decode($_POST['post0']);
           $map  = [
            ...
            //  'iNOM0SuJ'=>['table'=>'farmers','props'=>4],
        ...
        //print_r($map[$ob->find]);
           if($props ==1 ){
            $item_category    = DB::table($table)
              ->where("item_owner",$this->user->user_id)
           ...
           $item_category2   = [];
            ...
            $item_category    = DB::table($table)
            ->where("item_vendor",$this->user->user_id)
             ->where(function($DB)use($search){
            ...
            foreach ($item_category as $key => $value) {
              $value  = (array)$value;
                ...
                $item_category2[$key]['href_'] = '/farmer/product/view_detail__'.$value['item_id'];
