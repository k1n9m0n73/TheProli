...
class Profile extends Controller
{
 ...
 {  
    ...
    if (Auth::check()) {
        ...
        return view('farmer.component.settings.profile',['user'=>Auth::user()]);  
...
$chk = DB::table('farmers')->where('email', '!=',$this->user->email)->where('pn',$this->user->pn )->first();
...
try{ 
   ...
   $selected_state  = ['id'=>$selected_state_[0], 'zone_id'=> $selected_state_[1],'name'=>$selected_state_[2]];
            ...
            //   print_r($data);
            //   exit();
                DB::table('farmers')->where('user_id',$this->user->user_id)->update($data);
