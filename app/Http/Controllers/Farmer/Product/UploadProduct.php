     ...
     if (Auth::check()) {
        ...
        if (isset($_POST['product_category'])) {
           ...
           ///////////////////////item validation
            ...
            $checkStore  =  DB::table($map[\ucfirst( $req->input('partner_type2'))])->select('email')->where('user_id',$decode->partner_id)->first();
        ...
        if (! empty($checkStore) && $checkStore->email != $req->input('product_store') ) {
          ...
          // $owener_state = $ckf->st;  
            ...
            $partner_involve_code  =$storage_letter.'ff' ; //store.uploader.owner
           ////////////////////////////////////////
     ...
     /* sumer content is an aray 0 index is the fir content of summernote html elemet*/
          ...
          $setting  = DB::table('product_upload_setting')->where("id","1")->first();              
              $data  = [
               ...
               'item_vendor'=>$this->user->user_id,////////OWNER 
               'item_vendor_state'=>$this->user->st,
