...
window.addEventListener("load",function(){	
 ...
 function populateCate(ele,min,max){
     ...
     let wc = location.pathname.split("/category/items/")
     
      let item_subcategory = user().getData({ 
          appends:[JSON.stringify({"Id": wc[1].replace("/",""),
          ...
          "state":"<?=isset($_GET['state'])?$_GET['state']:null?>"}),min,max], url:"/getCategory",
                    ...
                    if($('.range-filter').length>0 ){
                        $( ".range-filter #slider-range" ).slider({
                            ...
                            slide: function( event, ui ) {
                                $( "#amount" ).html( "<span class='number'>" + ui.values[ 0 ] + "</span>" + " " + "<span class='number'>" + ui.values[ 1 ] + "</span>" );
...
/////////////////////////////////////////////////////////////////////type  function
 ...
 let category= user().getData({ 
        appends:[location.pathname.split("/")[3]],
