...
<div class="col-sm-12 col-md-12"    id="AnnTable" >
  ...
  <div class="card-header"   style="background-color:#89dd52;color:#fff;padding:6px">
     ...
     <form action="/farmer/product/upload"  department p__ method="post" mess__ class="item-contaner" >
      @csrf
                    <div class="wr" >  
                               ...
                               ptr.addEventListener('input',function(){
                                ...
                                // 
                                  if(typeof selectedType == 'undefined'){
                                    notify('error','The price of the item cannot be determined, contact admin about this item type',false,5000)
                                    return false;
                                  }
 ...
 //  popSubcate(id.value,id.children[1].innerText) 
   ...
   item_category.then(d=>{
        ...
        if(d.res.data.length==0){
           ...
           d.res.data.forEach(ca=>{
               cate+=`<option value="${ca.type_id}" fract="${ca.fraction}" style="text-transform: capitalize;"> ${ca.type_name}</option>`
           })
