...
<div class="col-sm-12 col-md-12"    id="AnnTable" >
  ...
  <div class="card-header"   style="background-color:#89dd52;color:#fff;padding:6px">
    ...
    <div class=" wr-" >  
                 <form enctype="multipart/form-data"  class="deal" >
                 @csrf
                <div class="typetotalField"><input type="hidden" name="id_" value=""></div>

                   <div class="wr" >
                            <h4>SET DEAL</h4>
                             <div class="form-group mt-3">
                                <label>Deal start on</label>
                       <div class="input-group">
                             
                             <input type="date" name="from_" class="form-control date-picker flatpickr-input" placeholder="Pick a date" 
                             is-req="" is-req-text="expiring date of is required" 
                             value="">
                           </div>
                         </div>    
                            ...
                            <div class="form-group mt-3">
                                <label>Deal end on</label>
                       <div class="input-group">
                        
                             <input type="date" name="to_" class="form-control date-picker flatpickr-input" placeholder="Pick a date" is-req="" is-req-text="expiring date of  is required" 
                             value="" >
                           </div>
                         </div>  
                       ...
                       <div class="input-group">
                         <div class="input-group-prepend">
                          <span class="input-group-text" style="background-color: #ccc"></span>
                             </div>
                             <input type="number" name="poff" step="0.01" min="0.1" max="0.9" class="form-control" placeholder="Percentage off"  is-req="" 
                             value=""> 
                           </div>
                         </div> 
                     ...
                     <button type="button" name="_send_deal_"   class="btn btn-theme" >Set deal <i class="fa fa-spinner anim" style="opacity: 0;"></i></button>
         ...
         window.addEventListener('load',function(){           
                function handleSetDeal(spinner,des_img_=null){
                ...
                let s =  pr.getData({
                    form:document.querySelector("form.deal"),
                    url:"/farmer/product/set_deal",
                    token:document.querySelector('form.deal').querySelector('input[name="_token"]').value,
                ...
                document.querySelector("button[name='_send_deal_']").addEventListener("click",function(){
                handleSetDeal(this)
                })
    ...
    <!-- ================================================================================================= -->
     ...
     <form action="/farmer/product/update"  department p__ method="post" mess__ class="item-contaner" >
      @csrf
                    <div class="wr" >  
                ...
                <div class="_itemtypetotalField"><input type="hidden" name="" value="1"></div>
                    ...
                    <div class="form-group mt-3">
                        ...
                        <input type="hidden" class="form-control" name="product_id"  autocomplete="off"  >
    ...
    <div class="wr" >
                      ...
                      ?>
                        <div class="form-group mt-3" style="width: 100%;">
                          ...
                          <input type="number"  step="0.01" name="product_price2" class="form-control" placeholder="Item price" 
                          aria-describedby="basic-addon1" is-req2__="" min="0" value="0" is-req-text="Iitem  name  1 is required"
                           ...
                           <input type="number" style="display: none;" step="0.01" name="product_price" class="form-control"
                           ...
                           <script type="text/javascript">
                             function setPriceAllow(itemTypes){
                                 let ptr  =   document.querySelector("input[name='product_price2']")
                     ...
                     ?>
                      <div class="form-group mt-3">
                              <label class="tag">Select one price</label>
                           <div class="input-group form-group grouper" style="margin: 0px auto;margin-top: 12px;height:  37.9px;margin-bottom: 10px;width: 100%">
                          <span class="input-group-addon" id="basic-addon1" style="border: none;font-size: 27px;color: #f00;background-color: transparent;padding-right: 10%;"> <i title="Add Item" class="proli-icon-naira" style="margin: 0px -9px;cursor: pointer;" add-type ></i></span>
 ...
 function upload_assistance(){
   ...
   let ptr  =   document.querySelector("[name='product_price']")
             ...
             ptr.addEventListener('change', function(){
                ...
                tag.innerText  = `You have decided to sell ${mass} ${unit} of this item for ₦${price} Recall, there is high competetion in market `
      ...
      setPrice:function(itemTypes ){
                        ...
                        el.addEventListener(ev,function(){
                          ...
                          let unit = document.querySelector("select[name='unit']").value;
                             ...
                             //console.log(_ID,location,path2)
                      ...
                      //console.log(this.files);
                       for (var i = 0; i < this.files.length; i++) {
                         ...
                         rd_2.onload = function(){
                           ...
                           img_f.classList =  $this.files.length>1? ['col-md-'+num_col+' col-sm-'+num_col+' col-xs-12 img-w ']: ['col-md-6 col-sm-6 col-xs-12 img-w'];
...
if ($data->allow_farmer_item_description == 1) {
?>
      <div class="wr">
...
?>
      ...
      <div class="form-control" style="border :none">  <button type="button" name="send_item"  is_item_upload class="btn btn-theme" style="margin: 0px -15px">Save <i class="fa fa-spinner anim" style="opacity: 0;"></i></button></div>
...
<script type="text/javascript"  >
     ...
     document.querySelector('form[p__]').setAttribute("done","")
     /////////////////////////
    console.log(id)
...
<script type="text/javascript">
    ...
    window.addEventListener("load",function (argument) {
        ...
        "/farmer/uploadImgUrl/usage_images_desc-pimg"/*uploade class_url/img_dir*/ ,"/usage/images/desc-pimg/",
   ...
   let item_id=  location.search.substr(1);
   function getItem(form ,callback){
 //   console.log(callback)
    
                      window.addEventListener("load",function(){

                        let userData = user().getData({
                            appends:['single',0,item_id],
                            url:"/farmer/product/process/detail_props",
...
////////////////////////////////////////////////////////////////////////////////////////
   ...
   getItem("[p__]",($data)=>{
    let t = setInterval(()=>{
          //////////////////////////////////////
           if(document.querySelector('form[p__]').hasAttribute("done")){ 
                console.log($data)
           ////////////////////////////////////////////////
          Array.from( document.querySelector('form[p__]').querySelector("select[name='product_category']").children).forEach(ch=>{
                    
                         if (ch.value == $data.data.d_ ) {
                          ch.setAttribute("selected","")
                           let ev  = new Event('change')
                           document.querySelector('form[p__]').querySelector("select[name='product_category']").dispatchEvent(ev)
                          //  chosenMaker.update2("select[name='re']")
                            ////////////////////////////////////////////////
                   setTimeout(()=>{ Array.from( document.querySelector('form[p__]').querySelector("select[name='product_subcatgory']").children).forEach(ch=>{
                                if (ch.value == $data.data.d ) {
                                ch.setAttribute("selected","")
                                let ev  = new Event('change')
                                document.querySelector('form[p__]').querySelector("select[name='product_subcatgory']").dispatchEvent(ev)

                                                        ////////////////////////////////////////////////
                            setTimeout(()=>{ Array.from( document.querySelector('form[p__]').querySelector("select[name='product_type']").children).forEach(ch=>{
                                            if (ch.value == $data.data.g ) {
                                            ch.setAttribute("selected","")
                                        ...
                                        document.querySelector("form[p__]").querySelector("input[name='product_price2']").dispatchEvent(e)
        ...
        document.querySelector("form[p__]").querySelector("input[name='product_name']").value  = $data.data.c
        document.querySelector("form[p__]").querySelector("input[name='mass']").value  = $data.data.r
        document.querySelector("form[p__]").querySelector("input[name='product_collection']").value  = $data.data.q2
        document.querySelector("form[p__]").querySelector("input[name='hd']").value  = $data.data.m_
        let price = $data.data.m==1?parseFloat( $data.data.k ): $data.data.k - parseFloat( $data.data.k )*parseFloat($data.data.m)
        document.querySelector("form[p__]").querySelector("input[name='product_price2']").value  =$data.data.m==1?$data.data.k:price 

        document.querySelector("form[p__]").querySelector("input[name='product_id']").value  = item_id
     
        document.querySelector("form.deal").querySelector("input[name='id_']").value  = item_id
        document.querySelector("form.deal").querySelector("input[name='id_']").value  = item_id
        document.querySelector("form.deal").querySelector("input[name='from_']").value  = $data.data.m==1?'':$data.data.j_7
        document.querySelector("form.deal").querySelector("input[name='to_']").value  = $data.data.m==1?'':$data.data.h_
        document.querySelector("form.deal").querySelector("input[name='poff']").value  = $data.data.m==1?'':$data.data.m
           ...
           ////////////////////////////////////////////////
           Array.from( document.querySelector('form[p__]').querySelector("select[name='product_state']").children).forEach(ch=>{
                         if (ch.value == $data.data.q4 ) {
                          ch.setAttribute("selected","")
                          //  chosenMaker.update2("select[name='re']")
                         }else{
                           ch.removeAttribute("selected")
        ...
        ////////////////////////////////////
           ...
           ////////////////////////////////////////////////
           Array.from( document.querySelector('form[p__]').querySelector("select[name='col']").children).forEach(ch=>{
                         if (ch.value == $data.data.q3 ) {
                          ch.setAttribute("selected","")
                          //  chosenMaker.update2("select[name='re']")
                         }else{
                           ch.removeAttribute("selected")
        ...
        ////////////////////////////////////
          ...
          ////////////////////////////////////////////////
          Array.from( document.querySelector('form[p__]').querySelector("select[name='unit']").children).forEach(ch=>{
                         if (ch.value == $data.data.s1 ) {
                          ch.setAttribute("selected","")
                          //  chosenMaker.update2("select[name='re']")
        ...
        ////////////////////////////////////
                   ...
                   let images  = JSON.parse($data.data.j_4).img   ;
                    //  console.log(images)
                      let img_tag =``
                      images.forEach((o,i)=>{
                     img_tag   += `<div class="col-md-${images.length==1?6:12/images.length} " >
                                        <span style="float:left" class="img_remove" title="Remove image below" ><i class="fa fa-close" ></i></span>
                                                <img src="/${o}"  formal-img style="width: 100%" class="img-w"  />  
                                                <input type="hidden" name="all_img[]" value="${o}">
                                            </div>
                                          
                                 </div>`
                      })
                      document.querySelector("._1fyLs_").innerHTML  =  img_tag 

     
 let img_remover =document.querySelectorAll("span.img_remove");
                       img_remover.forEach(re=>{
                        ...
                        try {
                            document.querySelector("div.note-editable.panel-body").innerHTML = `${JSON.parse($data.desc)[0]}`;
                        } catch (error) {
                            console.log(error)
                        }
                     

                      function handleDealCalculation(){

                           ////////////////////////
                           ['change','keyup'].forEach(ev=>{
               
                                            document.querySelector("input[name='poff']").addEventListener(ev,function(){
                                                let price2 = $data.data.m==1?parseFloat( $data.data.k ): ($data.data.k- parseFloat( $data.data.k )*parseFloat($data.data.m))
                                            console.log($data.data,"ertyui");
                                            let price = parseFloat(price2);
                                            let pf   = this.value
                                            let pl  = 1-this.value;
                                            let sp = pl*price;

                                            document.querySelector(".details").innerHTML = `initial price is ${price} Percentage off ${(pf*100).toFixed(2)}% 
                                            new selling price ${sp.toFixed(2)}`; 

                                            document.querySelector(".details").classList.add('btn-info') 
