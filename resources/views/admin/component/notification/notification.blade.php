            ...
            <div class="container-fluid">
                     ...
                     <div class="panel panel-bd lobidrag">
                         ...
                         <div class="panel-heading">
                             <div class="panel-title" style="max-width: calc(100% - 210px);">Add notification</div>
                           <button type="button" class="btn btn-xs btn-danger boxShadow pull-right" id="exitAddAnn"> Exit add notification</button>
                        </div>
                        <div class="panel-body">

                        <form action="/admin/notification/process/add-new"  department method="post" >
                            @csrf
                           <div class="form-content-wrapper" style="max-height:350px;position:relative;overflow-y:uto;" >
                            <span class="col-sm-12 col-md-6">
                            <div class="form-group t__ ">
                                <label class="control-label" for="username">Select Component</label>

                                <select class="select2 form-control"  name="comp">
                                        <option value="">Select partner to receive this mail</option>
                                        <option value="1">Aggregator</option>
                                        <option value="2">Farmer</option> 
                                        <option value="3">Logistics</option>
                                        <option value="4">Warehouse</option>
                                        <option value="5">Hub</option>
                                        <option  value="6">Admin</option>

                                 </select>
                            </div>
                            <div class="form-group o__ one__">
                            <label class="control-label" for="username">Enter user email and select which partner</label>
                                
                                <input type="text" name="one_user" class="form-control" placeholder="Enter the email of the user" />
                                <select class="select2 form-control" name="who">
                                        <option value="1">Aggregator</option>
                                        <option value="2">Farmer</option> 
                                        <option value="3">Logistics</option>
                                        <option value="4">Warehouse</option>
                            ...
                            <div class="form-group ">
                                <input type="checkbox" class="opt__" /> <span class="txt">Send to one user</span>
                            </div>
                          </span>
                         ...
                         <span class="col-sm-12 col-md-6">
                            <div class="form-group">
                                ...
                                <textarea cols="7" rows="37" name="cont" class="form-control" style="resize: none;"></textarea> 
                             </div>   
                          </span>

                      
                           <span class="col-sm-12 col-md-12  " style="top: 26px;">

                             <div class="form-group">
                               
                                <button type="button" name="adddistrict"   class="btn boxShadow department" >Save <i class="fa fa-spinner anim anim-for" style="opacity: 0;"></i></button>
                            </div>
                           </span>
                         </div>
                  ...
                  <form action="/admin/notification/process/add-new"  ty method="post" >
                            @csrf
                  </form>
...
document.querySelector(".opt__").addEventListener('change',function(){
    if(this.checked){
        document.querySelector(".o__").classList.remove("one__")
        document.querySelector(".t__").classList.add("two__")
        document.querySelector(".txt").innerText=" Send to many users"
    }else{
        document.querySelector(".o__").classList.add("one__")
        document.querySelector(".t__").classList.remove("two__")
        document.querySelector(".txt").innerText=" Send one one user"
    }
})
...
function respondTocal(){
  ...
  document.querySelector("button#exitAddAnn").click()
}
let a  = numCof();
 handleSubmission(
     "button.department",
     "form[department]",
     [JSON.stringify(a[0])],
     document.querySelector("form[department]").getAttribute("action"),
     respondTocal,
     'btn',
     {token:document.querySelector("form[department]").querySelector("input[name='_token' ]").value}
     );
                     ...
                     <div class="panel panel-bd lobidrag">
                        <div class="panel-heading" style="color: #fff">
                          <div class="panel-title" style="color: #fff">NOTIFICATION</div>
                               <button type="button" class="btn btn-xs btn-primary boxShadow pull-right" id="dropAddAnn">Add notification</button>
                        </div>
...
function getNot(){
  ...
  promise =  user().getData({
     url:"/admin/notification/process/get-not",
    appends:['weewe_','all'],
    ...
    let spinner = `<i class="fa fa-spinner anim" style="opacity: 1;"></i>`;
     function del_ () {
         ...
         user().getData({appends:[ev.target.getAttribute("index"),"456"],
            url:"/admin/notification/process/delNots",
            token:document.querySelector("form[ty]").querySelector("input[name='_token' ]").value
        }).then(data=>{
    ...
    let spinner = `<i class="fa fa-spinner anim" style="opacity: 1;"></i>`;
     function del_ () {
         ...
         user().getData({appends:[JSON.stringify(toDelete),"456"],
            url:"/admin/notification/process/delNotsMany",
            token:document.querySelector("form[ty]").querySelector("input[name='_token' ]").value
