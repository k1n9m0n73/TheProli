...
let p__ = `
   ...
   `
   let p__2=`${dropDownBox({list:["customer","farmer","aggregator","logistics","warehouse","hub"], activeVal:"customer" ,name:"component" })}
`
document.querySelector(".box").innerHTML  = p__

document.querySelector(".user").innerHTML  = p__2
 function i__(){
   //
 setTimeout(function(){
   ...
   let tablenew  = ` <table   class="table"  data-toggle="table-" data-pagination="true" data-search="true" data-show-columns="true" data-show-pagination-switch="true" data-show-refresh="true" data-key-events="true" data-show-toggle="true" data-resizable="true" data-cookie="true"
                                         ...
                                         <tr> 
                                              <th data-checkbox="true"></th>
                                                ...
                                                <th  data-field="User agent"  data-sortable="true"  data-editable="false">User agent</th>
 ...
 /* **Filter selecter****/
  ...
  let new_data = user().getData({
      appends:toAppend,url:"/"+component+"/analytics/process/event?"+numCof2[1]+"",
     ...
     //console.log($o.a_, "ID")
      list+=` <tr class="parent${$key}"  data-user="${$o.a_}" row-index=${$key+1}>
                                       <td class="1"  col-idex="1"></td>
                                        ...
                                        <td>${$o.b}</td>
                                        <td>${$o.c}</td>
                                        <td>${new Date(parseInt($o.d*1000) ).toString().substr(0,15) }</td>
                                        <td>${item.lat}</td>
                                        <td> ${item.lng}</td>
 ...
 dataSetPromise.then(dataPass=>{
  ...
  document.querySelector(table_r).querySelector('table.table').setAttribute("id",tblclass)
    let url ={
        flagout:"/"+component+"/analytics/process/event_delete_",
      add:"PARTH2",
      stm:"Are you sure to delete the selected event permanently",
      method:"POST",
      token:document.querySelector("input[name='_token']").value
