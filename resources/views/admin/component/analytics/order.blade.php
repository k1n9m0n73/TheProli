              ...
              <div class="table-responsive list-r">
                      ...
                      <div class="row">
                                ...
                                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 mt" >
                                    <div class="input-group">
                                        <span class="input-group-addon" id="basic-addon1">Search which log</span>
                         
                                    <select class="form-control" name="ltype" style="text-transform: capitalize;" cate> 
                                      <option value="">Select item</option>
                                      <option>Log1</option>
                                      <option>Log2</option>
                                      <option>Log3a</option>
                                    ...
                                    <input class="form-control year f-- two--" name="log_user" type="text" placeholder="Enter logistics is" />
                                </div>
...
let p__ = `
   ...
   `
   let p__2 = ` 
   ${dropDownBox({range:[20,80], activeVal:20,name:"Age from" })}
   ${dropDownBox({range:[20,80], activeVal:20,name:"Age to" })}
   ${dropDownBox({list:["Male","Felame"], activeVal:20,name:"Gender" })}
   
   ` 
   
   
document.querySelector(".box").innerHTML  = p__
document.querySelector(".box2").innerHTML  = p__2
 function i__(){
   //
 setTimeout(function(){
 ...
 let total_del_price  = 0;
    dataList.forEach( ($o,$key)=>{
      let log  =  document.querySelector("select[name  = 'ltype']").value
        let $del_price  = 0;
       switch (log) {
           case 'Log1':
              $del_price  = $o.m_
      ...
      list+=` <tr class="parent${$key}"  data-user="${$o.a}" row-index=${$key+1}>
                                        ...
                                        <td> ₦${  new Intl.NumberFormat('en',{maximumSignificantDigits:12}).format((  parseFloat($del_price)  ).toFixed(2))}</td>
 ...
 // if(getCookie('abpng__user__data_2')===data.res.cook ){
    ...
    document.querySelector("._tot-del").innerText=`₦${new Intl.NumberFormat('en').format(parseFloat(total_del_price).toFixed(1) ) } `
