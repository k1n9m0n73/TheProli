...
<div class="col-md-12 col-lg-12 col-sm-12 col-xs-12 " > 
			            ...
			            <div class="col-md-12 col-lg-12 col-sm-12 col-xs-12" >  
			                   ...
			                   <div class="col-md-12 col-lg-12 col-sm-12 col-xs-12 box-white" style="margin-bottom: 150px;background: #fff" > 
			                       <div class="form-group mt-3">
			                       	<input type="hidden" name="advid" value="<?=$ob->id?>"  />
			                       		<input type="hidden" name="filename" value="<?=$ob->image?>"  />
			                       		<input type="hidden" name="name_" value="<?=$ob->item?>"  />
			                              <label>EDITING ADVERT <?=strtoupper( $ob->item)?> </label>
			                             <img src="/<?=$ob->image?>" style="width: 100%;height:100%">
			                         </div>

			                          <div class="form-group mt-3">
			                              <label>Enter the header text</label>
			                              <input type="text" class="form-control" placeholder="Header text" autocomplete="off" name="ht" is-req="" is-req-text="Category name is required" value="<?=$ob->ht?>"  />
			   
			                         </div>

			                          <div class="form-group mt-3">
			                              <label>Enter the subheader text</label>
			                              <input type="text" class="form-control" placeholder="Subheader text" autocomplete="off" name="sht" is-req="" is-req-text="Category name is required" value="<?=$ob->sht?>" />
			   
			                         </div>


			                          <div class="form-group mt-3">
			                              <label>What to change image?</label>
			                                ...
			                                <select class="select2 select-other" name="item"  data-placeholder="Pick one or more options">
			                                	    ...
			                                	    <?php
			                                         ...
			                                         ?>
			        									...
			        									<option value="<?=$value->type_id?>"  <?=$ob->item==$value->type_name?'selected':''?>   cate-id ="<?=$value->category_id?>"  sub-id="<?=$value->subcategory_id?>"  ><?=$value->type_name?></option> 
			                                         <?php
			                                           }
			                                          ?>
			                          ...
			                          <div class="form-group mt-3" show-four  style="display: none;">
			                              <label>URL </label>
			                              <input type="text" class="form-control" url-add  autocomplete="off" name="url" is-req="" is-req-text="Category name is required" value="<?=$ob->url?>"  />
			                         </div>
...
let s =  pr.getData({form:document.querySelector("form.advert"),
    url:"/admin/sales_ads/process/edit"
    ,appends:[item_name,'update', ],
