               ...
               <div class="row">
                  <div class="col-sm-12">
                     <div class="mailbox">
                              ...
                              <div class="col-xs-4">
                                 <div class="inbox-avatar">
                                    ...
                                    <div class="inbox-avatar-text hidden-xs hidden-sm">
                                       <div class="avatar-name">Admin message box</div>
                                       <div><small></small></div>
                                    </div>
                                 </div>
                              </div>
                        ...
                        <form enctype="multipart/form-data" class="permission" x__ method="post"  action="/admin/message/process/sendMessage">
                            @csrf
...
function makeAction(callBack,form="form[x__]",
 action="/admin/message/process/delete-message",
...
function messages(){
  ...
  window.addEventListener('hashchange',getHashReader)
    ...
    function popData(form,which_ ,id,callback){
      ...
      let userData = user().getData({
          appends:['single',id],
          url:"/admin/message/process/"+which_,
          token:document.querySelector(form).querySelector("input[name='_token']").value,
 ...
 <table class="table  table-new-warehouse table_"  
...
popData('form[x__]',$messageData.msg=='inbox'?'inbox':'outbox', 'noid', ($data)=>{
     ...
     let $messages   =    $data.map((msg,inx)=>{ 
            ...
            $diff  = $diffs/60000;
            $hr  = $diff/60;
            console.log($diff,$diffs, $hr, parseInt(msg.h/60), $today_min,new Date(msg.i/1*1000).toString().substr(0,24))  
          
            if($diff < 60){
              $when_sent  = Math.ceil($diff)+" Min ago"
            }else if($hr < 24){
        ...
        function composeLinkClickEffect(){
             ...
             document.querySelector('[href="#compose"]').addEventListener('click',function($ele){
                ...
                handleSubmitWithSummer("button.__SEND__MESSAGE__","form[x__]",['farmers'],respondsToMessageSend,
                    "/admin/uploadImgUrl/usage_images_desc-message"/*uploade class_url/img_dir*/ ,"/usage/images/desc-message/",
