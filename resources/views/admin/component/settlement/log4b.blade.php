...
document.querySelector(".box").innerHTML  = p__
 ...
 /* **Filter selecter****/
  ...
  let new_data = user().getData({
      appends:toAppend,url:"/"+component+"/settlement/process/log4b?"+numCof2[1]+"",
 ...
 let  list  =``;
    dataList.forEach( (o,key)=>{
      ...
      list+=`     <tr class="parent${key}"  data-user="${o.a_}" row-index=${key+1}>
                                   ...
                                   <td>
                                   <button type="button" des="/${component}/settlement/process/log3b_gate_settle" action="Are you sure sure to settle by gateway " title="Gateway  Settlement" index="${key}" class="btn btn-xs btn-info delete-wh-r child-delete${key}"data-toggle="modal" data-target="#deleteschool2">
                                   <i class="fa fa-check-alt fa-check delete-wh-r" index="${key}" des="/${component}/settlement/process/log3b_gate_settle" action="Are you sure sure to settle by gateway " style="margin: 0px 0px;"></i> 
                                    </button>
 ...
 dataSetPromise.then(dataPass=>{
  ...
  document.querySelector(table_r).querySelector('table.table').setAttribute("id",tblclass)
    let url ={
      ...
      add:"PARTH2",
      stm:"Are you sure that this logistics 4b has been settled",
      method:"POST",
      token:document.querySelector("input[name='_token']").value
