...
document.querySelector(".box").innerHTML  = p__
   ...
   let tablenew  = ` <table   class="table"  data-toggle="table-" data-pagination="true" data-search="true" data-show-columns="true" data-show-pagination-switch="true" data-show-refresh="true" data-key-events="true" data-show-toggle="true" data-resizable="true" data-cookie="true"
                                         ...
                                         <th data-checkbox="true"></th>
                                           ...
                                           <!--       <th  data-field="unit cost" >Item Unit Cost</th>
                                               ...
                                               <th  data-field="VAT money" data-sortable="true">VAT money</th>
 ...
 /* **Filter selecter****/
  ...
  let new_data = user().getData({
      appends:toAppend,url:"/admin/settlement/process/vat?"+numCof2[1]+"",
 ...
 let  list  =``;
    dataList.forEach( (o,key)=>{
      ...
      list+=`     <tr class="parent${key}"  data-user="${o.a_}" row-index=${key+1}>
                                   ...
                                   <td>
                                   <button type="button" des="/${component}/settlement/process/service_settle_gate_vat" action="Are you sure sure to settle by gateway " title="Gateway  Settlement" index="${key}" class="btn btn-xs btn-info delete-wh-r child-delete${key}"data-toggle="modal" data-target="#deleteschool2">
                                   <i class="fa fa-check-alt fa-check delete-wh-r" index="${key}" des="/${component}/settlement/process/service_settle_gate_vat" action="Are you sure sure to settle by gateway " style="margin: 0px 0px;"></i> 
                                    </button>
 ...
 dataSetPromise.then(dataPass=>{
  ...
  document.querySelector(table_r).querySelector('table.table').setAttribute("id",tblclass)
    let url ={
        flagout:"/admin/settlement/process/service_settle_manual_vat",
      add:"PARTH2",
      stm:"Are you sure that this vat has been settled",
      method:"POST",
      token:document.querySelector("input[name='_token']").value
