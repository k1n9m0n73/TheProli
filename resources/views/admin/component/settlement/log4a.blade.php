...
document.querySelector(".box").innerHTML  = p__
   ...
   let tablenew  = ` <table   class="table"  data-toggle="table-" data-pagination="true" data-search="true" data-show-columns="true" data-show-pagination-switch="true" data-show-refresh="true" data-key-events="true" data-show-toggle="true" data-resizable="true" data-cookie="true"
                                         ...
                                         <th data-checkbox="true"></th>
                                           ...
                                           <!--       <th  data-field="unit cost" >Item Unit Cost</th>
                                              ...
                                              <th  data-field="Log1 ID" >Log1 ID</th>
                                               <th  data-field="Log1 amt" data-sortable="true">Delivery  cost</th>
 ...
 /* **Filter selecter****/
  ...
  let new_data = user().getData({
      appends:toAppend,url:"/"+component+"/settlement/process/log4a?"+numCof2[1]+"",
 ...
 let  list  =``;
    dataList.forEach( (o,key)=>{
      console.log(o.j)
        ...
        let bk_  = o.j?JSON.parse(o.j):{fn:'not available',ln:'not available',accountnumber:'not available',bankname:'not available'};
        console.log(item);
        let $imgs  = item.img
        let $img = $imgs || 'usage/demo/img/profile-pics/8.jpg';
      list+=`     <tr class="parent${key}"  data-user="${o.b}" row-index=${key+1}>
                                     ...
                                     <td>${bk_.hasOwnProperty('fn')?bk_.fn:''} ${bk_.ln?bk_.ln:''}  </td>  
                                    <td> ${bk_.accountnumber}  </td>  
                                       <td>${bk_.bankname}  </td>               
                                   
                                   <td>
                                   <button type="button" des="/${component}/settlement/process/log3a_gate_settle" action="Are you sure sure to settle by gateway " title="Gateway  Settlement" index="${key}" class="btn btn-xs btn-info delete-wh-r child-delete${key}"data-toggle="modal" data-target="#deleteschool2">
                                   <i class="fa fa-check-alt fa-check delete-wh-r" index="${key}" des="/${component}/settlement/process/log3a_gate_settle" action="Are you sure sure to settle by gateway " style="margin: 0px 0px;"></i> 
                                    </button>
                                    <button type="button" title="Manual Settlement" index="${key}" class="del__ btn btn-xs btn-primary delete-wh-r child-delete${key}"data-toggle="modal" data-target="#deleteschool"><i class="fa fa-check fa-check delete-wh-r" index="${key}" style="margin: 0px 0px;"></i> 
                                    </button>
 ...
 dataSetPromise.then(dataPass=>{
  ...
  document.querySelector(table_r).querySelector('table.table').setAttribute("id",tblclass)
    let url ={
      flagout:"/"+component+"/settlement/process/log4a_manual_settle",
      add:"PARTH2",
      stm:"Are you sure that this logistics 4a has been settled",
      method:"POST",
      token:document.querySelector("input[name='_token']").value
