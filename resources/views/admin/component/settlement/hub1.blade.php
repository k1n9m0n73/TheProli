...
document.querySelector(".box").innerHTML  = p__
   ...
   let tablenew  = ` <table   class="table"  data-toggle="table-" data-pagination="true" data-search="true" data-show-columns="true" data-show-pagination-switch="true" data-show-refresh="true" data-key-events="true" data-show-toggle="true" data-resizable="true" data-cookie="true"
                                         ...
                                         <th data-checkbox="true"></th>
                                           ...
                                           <!--       <th  data-field="unit cost" >Item Unit Cost</th>
                                              ...
                                              <th  data-field="Hub1 ID" >Hub1 ID</th>
 ...
 /* **Filter selecter****/
  ...
  let new_data = user().getData({
      appends:toAppend,url:"/"+component+"/settlement/process/hub1?"+numCof2[1]+"",
...
//let bk_  = o.j?JSON.parse(o.j):{fn:'not available',ln:'not available',accountnumber:'not available',bankname:''};
      ...
      list+=`     <tr class="parent${key}"  data-user="${o.a_}" row-index=${key+1}>
                                   ...
                                   <td>
                                   <button type="button" des="/${component}/settlement/process/hub1_gate_settle" action="Are you sure sure to settle by gateway " title="Gateway  Settlement" index="${key}" class="btn btn-xs btn-info delete-wh-r child-delete${key}"data-toggle="modal" data-target="#deleteschool2">
                                   <i class="fa fa-check-alt fa-check delete-wh-r" index="${key}"
                                    des="/${component}/settlement/process/hub1_gate_settle" action="Are you sure sure to settle by gateway " style="margin: 0px 0px;"></i> 
                                    </button>
 ...
 dataSetPromise.then(dataPass=>{
  ...
  document.querySelector(table_r).querySelector('table.table').setAttribute("id",tblclass)
    let url ={
      flagout:"/"+component+"/settlement/process/hub1_settle_manual",
      add:"PARTH2",
      stm:"Are you sure to that this hub 1 has been settle",
      method:"POST",
      token:document.querySelector("input[name='_token']").value
