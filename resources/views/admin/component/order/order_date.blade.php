...
document.querySelector(".box").innerHTML  = p__
 ...
 /* **Filter selecter****/
  ...
  let new_data = user().getData({
      appends:toAppend,url:"/"+component+"/order/process/order-order-date?"+numCof2[1]+"",
 ...
 let  list  =``;
    dataList.forEach( ($o,$key)=>{
      ...
      list+=` <tr class="parent${$key}"  data-user="${$o.a}" row-index=${$key+1}>
                                        ...
                                        <td> ${$o.s}</td>
                                     <td>
                                     <a style="background:#0"  href="/${component}/order/details__${$o.a}"><button type="button" class="del__ btn btn-xs btn-success  child-edit${$key}"><i class="zwicon-eye" ></i></button></a>
 ...
 dataSetPromise.then(dataPass=>{
  ...
  document.querySelector(table_r).querySelector('table.table').setAttribute("id",tblclass)
    let url ={
        flagout:"/"+component+"/order/process/delete_",
      add:"PARTH2",
