...
<script type="text/javascript">
 ...
 /* **Filter selecter****/
  ...
  let new_data = user().getData({
      appends:toAppend,url:"/admin/product/process/waiting-product?"+numCof2[1]+"",
 ...
 let  list  =``;
    dataList.forEach( ($o,$key)=>{
      ...
      list+=`<tr class="parent${$key}"  data-user="${$o.a}" row-index="${($key+1)}"> 
                                        ...
                                        <td > 
                                     <a   href="/admin/product/view_detail__${$o.a}"><button type="button" class=" btn btn-xs btn-success "><i class="zwicon-eye" ></i></button></a>
 ...
 dataSetPromise.then(dataPass=>{
  ...
  document.querySelector(table_r).querySelector('table.table').setAttribute("id",tblclass)
    let url ={
        flagout:"/admin/product/process/delete",
      add:"PARTH2",
