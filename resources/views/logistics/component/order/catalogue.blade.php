...
<div class="col-sm-12 col-md-12"    id="AnnTable" >
  <div class="panel panel-bd lobidrag">
     ...
     <form   department p__ method="post" forms >
      @csrf
        <!-- ./Plugin content:powerpoint,txt,pdf,png,word,xl -->
...
let p__ = `
   ...
   ${dropDownBox({range:[1,lastDayOfMonth], activeVal: parseInt(new Date().toISOString().substr(0,10).substr(8).trim()),name:"Day from" })}
    <br/>
   ...
   ${dropDownBox({range:[1,lastDayOfMonth], activeVal: parseInt(new Date().toISOString().substr(0,10).substr(8).trim()),name:"Day to" })}
...
document.querySelector(".box").innerHTML  = p__
 ...
 function table_($data,$otherData){
   let tablenew  = ` <table   class="table"  data-toggle="table-" data-pagination="true" data-search="true"
    data-show-columns="true" data-show-pagination-switch="true" data-show-refresh="true" data-key-events="true" 
    data-show-toggle="true" data-resizable="true" data-cookie="true"
                                         ...
                                         <tr> 
                                                    ...
                                                    <th data-field="Order logue ID">Order logue ID</th>
                                                    <th data-field="State from">State from </th>
                                                    <th data-field="City from">City from</th>
                                                    <th data-field="State to">State to </th>
                                                    <th  data-field="City to">City to </th>
 ...
 /* **Filter selecter****/
  ...
  let new_data = user().getData({
      appends:toAppend,url:"/logistics/order/process/catalogue?"+numCof2[1]+"",
 ...
 let  list  =``;
    dataList.forEach( (o,$key)=>{
      ...
      list+=` <tr class="parent${$key}"  data-user="${o.order_id}" row-index=${$key+1}>
                                           
                                        <td></td>
                                        <td>${new Date(parseInt(o.date*1000) ).toString().substr(0,15) }</td>
                                     ...
                                     <td>
                                     <button type="button" index="${$key}" class="del__ btn btn-xs btn-danger delete-wh-r child-delete${$key}"data-toggle="modal" data-target="#deleteschool"><i class="fa fa-close delete-wh-r" index="${$key}" style="margin: 0px 0px;"></i> 
                                      </button>
 ...
 dataSetPromise.then(dataPass=>{
  ...
  document.querySelector(table_r).querySelector('table.table').setAttribute("id",tblclass)
    let url ={
        flagout:"/logistics/order/process/delete_cata",
      add:"PARTH2",
      stm:"Are you sure to delete the selected item permanently",
      method:"POST",
      token:document.querySelector("input[name='_token']").value
