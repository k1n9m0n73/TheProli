 ...
 ?>     
      ...
      <div class="container">
            ...
            <!-- Login -->
                             ...
                             <?php
                                   ...
                                   $sz = json_decode($this->user->data()->sz);
                                      ...
                                      if ($this->user->data()->logistics_type   == 1  ||  $this->user->data()->logistics_type   == 2 ) {
                                        ...
                                        if ($sz[0] == "all") {
                          $sid =json_decode($this->user->data()->city)->country_id;
                          //print_r($sid);

                      $state  = DB::getInstance()->get2(['*'], "state",array("country_id = {$sid} AND name != '' ORDER BY name ")  );
                     ...
                     $sid =json_decode($this->user->data()->city)->country_id;
                      ...
                      $state  = DB::getInstance()->get2(['*'], "state",array("zone= ".$sz[0]." AND name != '' ORDER BY name ")  );
             ...
             <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 " style="margin-bottom: 36px;">
  ...
  <tbody>
<?php
  $che = (array)DB::getInstance()->get("log_des",array("log_id","=",$this->user->data()->log_id))->_results;
...
let btnPassReset = document.querySelector("button.bank_");
 ...
 promise =  user().getData({form:document.querySelector("#hub"),url:"<?=PATH.$this->table?>/finalDestination"});
