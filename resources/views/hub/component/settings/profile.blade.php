 ...
 <div class="container" style="margin-bottom: 13%;padding:12px">
       <div class="row" style="padding: 27px;margin-bottom: 38%">
         ...
         <div class="col-md-3 col-lg-3 col-sm-4 -col-xs-12">
                     ...
                     <div class="col-md-12 col-lg-12 col-sm-12 col-xs-12  content-pane-0" >  
                        ...
                        <div class="profile__img">
                            <img src="/<?=$img?>" alt="" style="    width: 50%;height: 50%;">
                             <form action="/hub/uploadProfileImage" enctype="multipart/form-data" method="post" class="profile-image" >
                               @csrf
                   ...
                   <span class="card-title " style="color: #e33;margin-bottom: -2px;;" ><?=$user->fn?> profile</span>
                               
                           </div>
                           
                 
                     
                   <form enctype="multipart/form-data" action="/hub/addCategory" class="register" >
                     @csrf
                    <div class="totalSubcategoryField"><input type="hidden" name="totalSubcategoryField" value="1"></div>
                      ...
                      <div class="col-md-12 col-lg-12 col-sm-12 col-xs-12 box-white" >
                           <h4 style="color: #000;margin: 0 29px">{{$user->fn}} information</h4>

                           <div class="form-group mt-3">
                                <label>First  name</label>
                                <input type="text"  name="fn" value="{{$user->fn}}"  class="form-control" placeholder="First name" autocomplete="off" is-req="" is-req-text="Business name is required"  >
                           </div>

                           <div class="form-group mt-3">
                                <label>Last  name</label>
                                <input type="text"  name="ln" value="{{$user->ln}}"  class="form-control" placeholder="Last name" autocomplete="off" is-req="" is-req-text="Business name is required"  >
                           </div>
                          ...
                          <script type="text/javascript">
                              document.querySelector("div[lga-opt2]").innerHTML =`<select data-placeholder="Select state"class="select2 states" 
                              name="lga"><option selected values="<?=json_decode($user->state)->id?>" lga-name="<?=json_decode($user->lga)->lga_name?>"
                               value="<?=json_decode($user->state)->id?>__#__<?=json_decode($user->lga)->lga_name?>"><?=json_decode($user->lga)->lga_name?>
                               </option></select>`
                                               
                             document.querySelector("div[area-opt2]").innerHTML =`<select data-placeholder="Select state"class="select2 states" 
                              name="area"><option selected
                              value="<?=json_decode($user->lga)->latitude?>__#__<?=json_decode($user->lga)->longitude?>__#__<?=json_decode($user->lga)->area_name?>"><?=json_decode($user->lga)->area_name?></option><select>`   
                              ...
                              try{
                                 Array.from(document.querySelector("div[state-opt2]").querySelector("select").children).forEach(op=>{
                                              //   console.log(op, op.innerText,"<?=$user->st?>")
                                                   if (op.innerText === "<?=json_decode($user->state)->name?>") {
...
document.querySelector("button[name='send_form']").addEventListener("click",function(){
   request.sendForm(this,document.querySelector("form.register"),"/hub/settings/profile/update") 
})
...
document.querySelector("button[name='uploadprofileimage']").addEventListener("click",function(){
   ...
   request.sendForm(this,document.querySelector("form.profile-image"),"/hub/settings/profile/update-profile-image", setImage) 
})
