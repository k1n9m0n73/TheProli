    ...
    tableExport: function (options) {
      ...
      if ( defaults.type == 'csv' || defaults.type == 'tsv' || defaults.type == 'txt' ) {
        ...
        try {
          ...
          //  document.querySelector("button.buttons-excel").click();
     ...
     let   $this = this //////////the table element
     

            function runImgToBase64(callback_runPdf){



         function imgToBase64(src, callback,callback2=null) {
...
console.log(allImages)
   ...
   if (allImages.length>0) {
    ...
    for (let k = 0; k < allImages.length; k++) {
      timeConstant++;
  let src = allImages[k].getAttribute("src");

 imgToBase64(src,function(dataURI){

    allImages[k].setAttribute('src',dataURI) //console.log(datas)
}) 

 if((allImages.length-1) == k){
  setTimeout(function(){
   callback_runPdf(); 
  },2000)
...
return false;
 ...
 runImgToBase64(runPdf)
        ...
        if ( defaults.pdfmake.enabled === true ) {
          ...
          loadImages(teOptions, function () {
            ...
            teOptions.columns.forEach(function(r){
              try{
                  if(r.title==='') {
                    r.title="Check"
                  }
                  headObj[(r.title).toLowerCase()] =(r.title)
 ...
 ////////////////////////////////////////////////
            ...
            //  teOptions.doc .autoTable(teOptions.columns, teOptions.rows, atOptions);
              ...
              let selectedRow  =  Array.from($this.querySelector("tbody").querySelectorAll("tr.selected"))
              
                   if (selectedRow.length>0 && $this.offsetParent.offsetParent.querySelector("select.dt-tb").value =="selected") {

                  Array.from($this.querySelector("tbody").querySelectorAll("tr")).forEach( (tr,num)=>{
...
//////////////////////////////////////style first row
   ...
   Array.from($this.querySelector("tbody").querySelectorAll("tr")[0].querySelectorAll('td')).forEach(td=>{
     td.setAttribute("style","border:1px solid #eee")
     utf8Encode(td.innerText)
     base64encode(td.innerText)
   })

   Array.from($this.querySelector("thead").querySelectorAll("tr")[0].querySelectorAll('th')).forEach(td=>{
     td.setAttribute("style","border:1px solid #eee")
   })



   //    Array.from($this.querySelector("tbody").querySelectorAll("tr")).forEach(tr=>{
   //    if ( tr.querySelectorAll('td')[0].className.match("bs-checkbox")) {
   //    tr.querySelectorAll('td')[0].remove()/////remove first column  
   //    }
   //  // tr.querySelectorAll('td')[0].remove()/////remove first column
   // })

   //  Array.from($this.querySelector("thead").querySelectorAll("tr")).forEach(tr=>{
   //    if ( tr.querySelectorAll('th')[0].className.match("bs-checkbox")) {
   //    tr.querySelectorAll('th')[0].remove()/////remove first column  
   //    }
...
//   });
                 ...
                 let tableObject  = {
                   ...
                   // body:teOptions.rows,
                    ...
                    // styles: {cellPadding: 1.5, fontSize: 2,}, 
                   
          

                didDrawPage: function (data) {
                     var pageSize =  teOptions.doc.internal.pageSize
                  var pageWidth = pageSize.width ? pageSize.width : pageSize.getWidth()
                  var text = teOptions.doc.splitTextToSize($this.offsetParent.offsetParent.parentElement.querySelector('.panel-title').innerText, pageWidth - 30, {})   
                                         // console.log(data,teOptions.images)
                  teOptions.doc.text(text, 14, 40)///table titl
                  //teOptions.doc.text($this.offsetParent.offsetParent.parentElement.querySelector('.panel-title').innerText, 14, 40)
       ...
       didDrawCell: (data) => {
    if (data.section === 'body' && (data.column.index === 1 || data.column.index === 0 )) {
     ...
     // var base64Img = 'data:image/jpeg;base64,iVBORw0KGgoAAAANS...'
      data.doc.addImage(img.src, textPos.x+0.5,  textPos.y-4, data.cell.width-3, data.cell.height-3);
}
      ...
      let check_is_select = false;

   


               teOptions.doc.autoTable(tableObject );



             teOptions.doc.save('Exported-data.pdf')




             Array.from($this.querySelector("tbody").querySelectorAll("tr")).forEach(td=>{
     td.removeAttribute("style")
              ...
              defaults.jspdf.autotable.startY = teOptions.doc.previousAutoTable.finalY + atOptions.margin.top;
